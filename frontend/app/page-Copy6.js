...
export default function Home() {
    ...
    const poll = async () => {
      try {
        ...
        if (jobStatus === "done") {
          ...
          setErroredJobs(loadErroredJobsFromStorage());
          // Remove from analyzingJobs, etc.
          const jobs = loadAnalyzingJobsFromStorage().filter(j => j.jobId !== job.jobId);
          saveAnalyzingJobsToStorage(jobs);
  ...
  const onDrop = async (acceptedFiles) => {
    ...
    for (let i = 0; i < acceptedFiles.length; i++) {
      ...
      debugLog('Uploading with customRedFlags:', customRedFlags);
      ...
      try {
        ...
        pollJobUntilDone(job); // Start polling immediately
      } catch (err) {
                      ...
                      <Grid item xs={12} sm={6} md={4} key={rep}>
                        <Card elevation={3} sx={{ borderRadius: 3 }}>
                          <CardContent>
                            ...
                            <Chip
                              label={`Rep talks for: ${avgTalkListen !== null ? Math.round(avgTalkListen * 100) + '%' : 'N/A'}`}
                              color="primary"
                              sx={{ mr: 1 }}
                            />
                            <Chip
                              ...
                              <Box sx={{ mb: 1, pl: 1 }}>
                                <ul style={{ margin: 0, paddingLeft: 16 }}>
                                  {dl.custom_red_flags_found.map((flag, idx) => (
                                    <li key={idx} style={{ color: '#b71c1c', fontSize: '0.95em' }}>{flag}</li>
                                  ))}
                                </ul>
                              </Box>
                            )}
