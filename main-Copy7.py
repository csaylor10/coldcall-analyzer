...
async def split_audio(filepath, chunk_length_sec=600):
    """
...
async def process_audio_job(job_id, filepath, params, user_id, custom_red_flags):
    ...
    try:
        ...
        logger.info(f"Audio file {filepath} duration: {duration:.2f} seconds")
        chunk_limit_sec = 900  # 15 min per chunk (adjust for your API)
        if duration > chunk_limit_sec:
            logger.info(f"Splitting {filepath} into chunks of {chunk_limit_sec} seconds...")
            chunk_paths = await split_audio(filepath, chunk_length_sec=chunk_limit_sec)
        else:
            chunk_paths = [filepath]

        # --- Transcribe/analyze each chunk ---
        transcripts = []
        for chunk_path in chunk_paths:
            logger.info(f"Transcribing/analyzing chunk: {chunk_path}")
        ...
        # Formatting transcript for analysis
        format_prompt = f'''
Organize the following transcript into a structured conversation.
...
'''
        ...
        logger.info(f"Formatted transcript for analysis.")

        # --- RED FLAG LOGIC ---
        if custom_red_flags:
            red_flags_str = "\n- ".join(custom_red_flags)
            msg = f"Check ***ONLY*** these explicit red flags:\n- {red_flags_str}"
        else:
            msg = "Check ***ONLY*** these explicit red flags:"
        eval_prompt_dynamic = EVAL_PROMPT_TEMPLATE.replace(
            "Check ***ONLY*** these explicit red flags:",
            msg
        )
        analysis_prompt = eval_prompt_dynamic.format(transcript=formatted_transcript, COMPANY=COMPANY)
        logger.debug(f"Analysis prompt for job {job_id}: {analysis_prompt}")
        ...
        logger.info(f"OpenAI analysis complete for job {job_id}.")

        # --- Extract details ---
        negative_responses = [
            r"\bno\b", r"\bnope\b", r"\bnah\b", r"\bnot interested\b", r"\bno thanks?\b", r"\bnot now\b",
            r"\bnot today\b", r"\bnot at this time\b", r"\bnot really\b", r"\bno way\b", r"\bnever\b",
            r"\bdon't want\b", r"\bdon't need\b", r"\bdon't think so\b", r"\bi can't\b", r"\bcan't\b",
            r"\bwon't\b", r"\bi won't\b", r"\bdecline\b", r"\bpass\b", r"\bnot going to\b", r"\bno can do\b",
            r"\bnot for me\b", r"\bno sir\b", r"\bno ma'am\b", r"\bno thank you\b"
        ]
        accept_responses = [
            r"\byes\b", r"\byep\b", r"\byup\b", r"\bokay\b", r"\bok\b", r"\bsure\b", r"\ball right\b",
            r"\bthat works\b", r"\bsounds good\b", r"\bgreat\b", r"\bperfect\b", r"\bgo ahead\b",
            r"\blet's do it\b", r"\bi'll take it\b", r"\bi accept\b", r"\bi agree\b", r"\babsolutely\b",
            r"\bwhy not\b", r"\bof course\b", r"\bcount me in\b", r"\bdeal\b", r"\bfine\b", r"\baccepted\b"
        ]

        def contains_any(phrases, text):
            return any(re.search(phrase, text, re.I) for phrase in phrases)
        ...
        for line in formatted_transcript.splitlines():
            ...
            if line_lower.startswith("rep:"):
                rep_words += len(line.split())
            elif line_lower.startswith("customer:"):
                if not found_accept:
                    if contains_any(negative_responses, line_lower):
                        nos_before_accept += 1
                    if contains_any(accept_responses, line_lower):
                        found_accept = True
                customer_words += len(line.split())

        talk_to_listen_ratio = rep_words / customer_words if customer_words else None
        ...
        call_type = re.search(r"Call Categorization:\s*(.+)", analysis_text)
        call_type = call_type.group(1).strip() if call_type else "UnknownType"
        call_type_sanitized = call_type.replace(" ", "_")
        red_flags = re.search(r"Red Flags:\s*(Yes|No)", analysis_text, re.I)
        red_flags = red_flags.group(1).capitalize() if red_flags else "No"
        red_flag_reason = re.search(r"Reason for Red Flag:\s*(.+)", analysis_text)
        red_flag_reason = red_flag_reason.group(1).strip() if red_flag_reason else "None"
        red_flag_quotes = re.search(r'Red Flag Direct Quotes:\s*\["(.+)"\]', analysis_text)
...
""" 
        ...
        async with async_session() as session:
            ...
            job.result = {
                ...
                "categorization": call_type_sanitized,
                "red_flags": red_flags,
                ...
                "nos_before_accept": nos_before_accept
            }
            await session.commit()
        ...
        async with async_session() as session:
            ...
            await session.commit()
    elapsed = time.perf_counter() - start_time
    logger.info(f"process_audio_job for {job_id} took {elapsed:.2f} seconds")
