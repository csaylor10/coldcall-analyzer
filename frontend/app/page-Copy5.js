...
export default function Home() {
        ...
        try {
          ...
          if (jobStatus === "done") {
            ...
            if (!downloads.some(dl => dl.jobId === job.jobId)) {
              ...
              // Use just the number for overall_perf, no 'of10', and detailed filename
  ...
  useEffect(() => {
    ...
    const handleDragLeave = e => {
      if (e.relatedTarget === null || e.relatedTarget === document.body) setDragActive(false);
    };
    const handleDrop = e => {
      e.preventDefault();
      setDragActive(false);
      // Only trigger if not dropping on upload zone
      if (uploadZoneRef.current && uploadZoneRef.current.contains(e.target)) return;
      if (e.dataTransfer?.files?.length) {
        try {
          const files = Array.from(e.dataTransfer.files);
          onDrop(files);
        } catch (err) {
          setUploadError('Drag-and-drop error: ' + (err?.message || err));
        }
      }
    };
  ...
  return (
    <ThemeProvider theme={createTheme({
      palette: {
        ...
        ...(darkMode ? {
          background: { default: '#151a23', paper: '#1d2330' },
          text: { primary:"#e0e0e0", secondary:"#b0c4de" },
          primary: { main: "#00D1FF" },
          success: { main: "#00C36F" },
        } : {
          background: { default: "#f4f6fa", paper: "#fff" },
          text: { primary:"#0f2a54", secondary:"#7e8ba3" },
        })
      },
      components: {
        MuiCard: {
          styleOverrides: {
            root: {
              background: darkMode ? "#1d2330" : "#fff",
              color: darkMode ? "#e0e0e0" : "#0f2a54",
            }
          }
        },
        MuiDrawer: {
          styleOverrides: {
            paper: {
              background: darkMode ? "#1a202b" : "#fff",
              color: darkMode ? "#e0e0e0" : "#0f2a54",
            }
          }
        }
      ...
      {jwt && (
        <Drawer
          ...
          sx={{
            ...
            bgcolor: darkMode ? "#1a202b" : "#f8fafc",
        ...
        >
          <Toolbar sx={{ minHeight: 56, p: 0 }} />
          <Box sx={{ px: 2, py: 2, display: 'flex', flexDirection: 'column', alignItems: 'flex-start', borderBottom: '1px solid #e0e4ea', gap: 2 }}>
            {/* User avatar/profile */}
            <Box sx={{ display: 'flex', alignItems: 'center', gap: 1, mb: 1 }}>
              <AccountCircleIcon sx={{ fontSize: 36, color: "#7e8ba3" }} />
              <Box>
                <Typography sx={{ fontWeight: 700, fontSize: 16, color: "#0f2a54" }}>{COMPANY}</Typography>
                {userEmail && <Typography sx={{ fontSize: 13, color: "#7e8ba3" }}>{userEmail}</Typography>}
              </Box>
            </Box>
            {/* Minutes & Buy */}
            <Box sx={{ display: 'flex', flexDirection: 'column', alignItems: 'flex-start', gap: 1, mb: 2 }}>
              <Chip
                ...
                sx={{
                  bgcolor: darkMode ? "#232c3b" : "#f5f7fa",
              ...
              <Button
                variant="contained"
                color="success"
                sx={{ mt: 0, borderRadius: 99, fontWeight: 600, fontSize: 13, minWidth: 120 }}
            ...
            {/* Dark mode toggle */}
            <Button startIcon={darkMode ? <WbSunnyIcon /> : <NightlightIcon />} onClick={() => setDarkMode(v => !v)} sx={{ mt: 1, borderRadius: 99, fontWeight: 600, fontSize: 13 }}>
              {darkMode ? 'Light Mode' : 'Dark Mode'}
            </Button>
          </Box>
          <List sx={{px: 1, pt: 2, gap: 2}}>
            {/* Red Flags Section */}
            <Tooltip title="Custom Red Flags" placement="right" arrow disableHoverListener={true}>
              <ListItem disablePadding sx={{ display: 'block', borderRadius: 2, mb: 1, bgcolor: redFlagsOpen ? 'rgba(0,211,255,0.08)' : 'transparent', borderLeft: redFlagsOpen ? '4px solid #00D1FF' : '4px solid transparent', transition: 'all 0.2s' }}>
                <ListItemButton onClick={() => setRedFlagsOpen(!redFlagsOpen)} sx={{ minHeight: 44, borderRadius: 2, px: 2.5, gap: 1.5 }}>
                  <FlagRoundedIcon sx={{ color: "#00D1FF", fontSize: 22 }} />
                  <Typography variant="overline" sx={{ fontWeight: 700, letterSpacing: 1, textTransform: 'uppercase', fontSize: 13, ml: 0.5 }}>
                    Custom Red Flags
                  </Typography>
                  <Box sx={{ flexGrow: 1 }} />
                  {redFlagsOpen ? <ChevronDownIcon sx={{ color: "#9db2c8", fontSize: 18, ml: 0.5 }} /> : <ChevronRightIcon sx={{ color: "#9db2c8", fontSize: 18, ml: 0.5 }} />}
                </ListItemButton>
              </ListItem>
            </Tooltip>
            <Collapse in={redFlagsOpen} timeout={300} unmountOnExit>
              <Box sx={{ px: 3, py: 2, bgcolor: darkMode ? "#232c3b" : "#fff", borderRadius: 2, boxShadow: '0 2px 8px 0 rgba(30,40,90,0.03)' }}>
                <TextField
                  ...
                  size="small"
                  fullWidth
                  sx={{ mb: 1, bgcolor: darkMode ? "#232c3b" : "#f5f7fa", borderRadius: 2, '& .MuiOutlinedInput-root': { fontSize: 14, height: 36 } }}
                ...
                <Box sx={{ display: 'flex', flexWrap: 'wrap', gap: 1, minHeight: 24 }}>
                  {customRedFlags.map((flag, idx) => (
                    <Chip
                      ...
                      sx={{
                        bgcolor: darkMode ? "#232c3b" : "#f5f7fa",
                        color: darkMode ? "#e0e0e0" : "#2b3a55",
                        border: '1px solid #e0e4ea',
                        fontWeight: 600,
                        fontSize: 13,
                        height: 28,
                        transition: 'all 0.2s',
                        '&:hover': { bgcolor: darkMode ? "#1a202b" : "#e3e8f0", borderColor: '#b6c3d1' },
                        boxShadow: '0 1px 3px 0 rgba(30,40,90,0.03)',
                      }}
                      variant="outlined"
                    />
                  ))}
            ...
            <Tooltip title="Current Reps" placement="right" arrow disableHoverListener={true}>
              <ListItem disablePadding sx={{ display: 'block', borderRadius: 2, mb: 1, bgcolor: repsOpen ? 'rgba(0,211,255,0.08)' : 'transparent', borderLeft: repsOpen ? '4px solid #00D1FF' : '4px solid transparent', transition: 'all 0.2s' }}>
                <ListItemButton onClick={() => setRepsOpen(!repsOpen)} sx={{ minHeight: 44, borderRadius: 2, px: 2.5, gap: 1.5 }}>
                  <PeopleAltRoundedIcon sx={{ color: "#0F2A54", fontSize: 22 }} />
                  <Typography variant="overline" sx={{ fontWeight: 700, letterSpacing: 1, textTransform: 'uppercase', fontSize: 13, ml: 0.5 }}>
                    Current Reps
                  </Typography>
                </ListItemButton>
        ...
        {!jwt ? (
          ...
          <Box sx={{ bgcolor: "background.default", minHeight: "100vh" }}>
            <Container maxWidth="lg">
                ...
                <CardContent sx={{ p: { xs: 2, sm: 4 }, display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 2 }}>
                  ...
                  >
                    ...
                    <Box sx={{ display: 'flex', alignItems: 'center', gap: 0.5, mb: 1 }}>
                      <Typography variant="body2" sx={{ color: darkMode ? "#b0c4de" : "#6b7a90", fontFamily: 'Roboto, sans-serif' }}>
              ...
              {/* --- In Progress Section: ONLY here, below the buttons and above Download Reports --- */}
      ...
      <Snackbar open={toast.open} autoHideDuration={4000} onClose={() => setToast({ ...toast, open: false })} anchorOrigin={{ vertical: 'bottom', horizontal: 'center' }}>
        <MuiAlert elevation={6} variant="filled" onClose={() => setToast({ ...toast, open: false })} severity={toast.severity} sx={{ fontWeight: 600 }}>
          {toast.message}
        </MuiAlert>
      </Snackbar>
      {uploadError && (
        <Snackbar open={!!uploadError} autoHideDuration={6000} onClose={() => setUploadError('')} anchorOrigin={{ vertical: 'top', horizontal: 'center' }}>
          <MuiAlert elevation={6} variant="filled" severity="error" onClose={() => setUploadError('')} sx={{ fontWeight: 600 }}>
            {uploadError}
          </MuiAlert>
        </Snackbar>
      )}
