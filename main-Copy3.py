...
async def upload_audio(file: UploadFile = File(...)):
    ...
    # Reformat transcript explicitly
    format_prompt = f"""
    Organize the following transcript into a structured conversation between "Thomas" and "Client":
    Label explicitly each line:
    ...
    if not formatted_transcript:
        os.remove(filepath)
        raise HTTPException(status_code=500, detail="Transcript formatting failed.")
    ...
    analysis_filename = (
        f"{extracted['rating']}_"
        f"{extracted['rep_name']}_"
        f"{extracted['call_type']}_"
        f"RedFlags{extracted['red_flags']}_"
        f"{file.filename}_analysis.txt"
    )
    ...
    if not full_analysis_content.strip():
        os.remove(filepath)
        raise HTTPException(status_code=500, detail="Generated content empty.")

    # Save analysis explicitly to ZIP
    with zipfile.ZipFile(zip_filename, "a") as zipf:
        zipf.writestr(analysis_filename, full_analysis_content)

    os.remove(filepath)

    # Return all required fields explicitly (matching frontend needs)
    return {
